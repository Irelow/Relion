...
module.exports = async function handler(req, res) {
        ...
        html: `
          <div style="font-family: -apple-system, sans-serif; max-width: 520px; margin: 0 auto; color: #1a1a1a;">
            ...
            <div style="background: #fff; padding: 32px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
              ...
              <p>Votre abonnement Relion est actif. Voici vos identifiants :</p>
              <div style="background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 20px; margin: 24px 0;">
                <p style="margin: 0 0 8px 0;"><strong>Email :</strong> ${email}</p>
                <p style="margin: 0;"><strong>Mot de passe :</strong>
                  <span style="font-family: monospace; font-size: 20px; letter-spacing: 3px; color: #1A56E8; font-weight: bold;">${tempPassword}</span>
                </p>
              </div>
              <p>👉 <a href="https://relionapp.fr" style="color: #1A56E8; font-weight: bold;">Connectez-vous sur relionapp.fr</a></p>
  ...
  if (event.type === 'customer.subscription.deleted') {
    ...
    try {
      await sql`UPDATE users SET subscription_status = 'canceled' WHERE stripe_subscription_id = ${subscription.id}`;
    } catch (err) {
